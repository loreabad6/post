...
polygons = rbind(
  st_sf(gid = letters[1:5], tid = 1, geometry = pol_t0),
  st_sf(gid = letters[1:5], tid = 2, geometry = pol_t1),
  st_sf(gid = letters[1:5], tid = 3, geometry = pol_t2),
  st_sf(gid = letters[1:5], tid = 4, geometry = pol_t3),
  st_sf(gid = letters[1:5], tid = 5, geometry = pol_t4)
) |> st_set_crs(4326)

# Save as data for the package
usethis::use_data(polygons)
