...
```
With this in mind, we can also create a new dimension with the result of a binary predicate geometric operation.
Note that we need to use the `sparse = TRUE` option to obtain a boolean array from the operation. 
Since this is a new attribute in our cube, the dimensions are preserved.

```{r}
arr$int_circle = st_intersects(arr[['geometry']], circle, sparse = FALSE)
arr
```
