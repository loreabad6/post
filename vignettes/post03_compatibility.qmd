...
#### 1. `post_table` cannot handle objects with 0 rows.

Coming from an [issue in `cubble`](https://github.com/huizezhang-sherry/cubble/issues/35), objects of class `cubble_df` return an error when no rows are present.
For example if we perform a filtering operation as:
...
This also means that if a filtering or subsetting operation is applied to a `post_array`, which can handle emptiness, when converting to a `post_table` we get an error.
